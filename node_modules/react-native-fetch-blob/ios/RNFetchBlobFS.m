...
+ (void) emitDataChunks:(NSData *)data encoding:(NSString *) encoding streamId:(NSString *)streamId event:(RCTEventDispatcher *)event
{
    ...
    if([[encoding lowercaseString] isEqualToString:@"big5"])
    {        
        NSLog(@"grimmer modified");        
        
        // NSStringEncoding big5 = CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingBig5_E);
        // NSString *string = [[NSString alloc] initWithData:data encoding:big5]; 

        CFStringRef r = CFStringCreateWithBytes(NULL, [data bytes],[data length], kCFStringEncodingBig5_HKSCS_1999, true);   
		NSString *string = (__bridge NSString *)r;

        // NSLog(@"grimmer after modified:%@", string); //sometimes it is null        
        NSLog(@"grimmer after modified"); //sometimes it is null        
        if(string==nil){
       		NSLog(@"grimmer is null"); //sometimes it is null 
       		string = @"";       
        }

        NSDictionary * payload = @{ @"event": FS_EVENT_DATA,  @"detail" : string};
        [event sendDeviceEventWithName:streamId body:payload];
    }
